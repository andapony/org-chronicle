...
(ert-deftest org-chronicle-test-remove-lane-prompt ()
  ...
  (let ((org-chronicle--view-args (list :people '("Ada" "Bob")))
    ...
    (cl-letf (((symbol-function 'org-chronicle--lane-at-point) (lambda (&optional _p) nil))
              ((symbol-function 'completing-read-multiple) (lambda (&rest _) '("Ada [person]")))
              ((symbol-function 'org-chronicle-view-refresh)
               (lambda () (setq refreshed t))))
      (org-chronicle-view-remove-lane)
...
(ert-deftest org-chronicle-test-remove-lane-prompt-disambiguates-domain ()
  "The remove prompt distinguishes same-named lanes across domains."
  (let ((org-chronicle--view-args (list :people '("Washington")
                                        :locations '("Washington")))
        (refreshed nil))
    (cl-letf (((symbol-function 'org-chronicle--lane-at-point) (lambda (&optional _p) nil))
              ((symbol-function 'completing-read-multiple)
               (lambda (&rest _) '("Washington [place]")))
              ((symbol-function 'org-chronicle-view-refresh)
               (lambda () (setq refreshed t))))
      (org-chronicle-view-remove-lane)
      (should (equal (plist-get org-chronicle--view-args :people) '("Washington")))
      (should (null (plist-get org-chronicle--view-args :locations)))
      (should refreshed))))
